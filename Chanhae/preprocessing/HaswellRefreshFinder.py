...
if __name__ == '__main__':
    ...
    f = open('./codenameFolder/하스웰(4세대,리프레시).txt', 'r', encoding='utf-8')
    lines = f.readlines()
    for line in lines:
        code = line.split()[-1].replace('/B','').replace('/A','').replace('(','').replace(')','')
        ...
        for x in var:
            if x in row[2]:
                if str(data2.iat[index,6]) == 'nan':
                    data2.iat[index, 6] = '하스웰(4세대,리프레시)'
