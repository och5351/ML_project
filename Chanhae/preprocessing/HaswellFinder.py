...
if __name__ == '__main__':
    # 하스웰(4세대) 제품군 코드 담는 배열
    var = []
    # 하스웰(4세대) 기입되어있는 name data open
    f = open('./codenameFolder/하스웰(4세대).txt', 'r', encoding='utf-8')
    lines = f.readlines()
    for line in lines:
        code = line.split()[-1].replace('/A','').replace('특가판매','').replace('(','').replace(')','')
